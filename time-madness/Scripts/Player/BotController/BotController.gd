...
# 🔥 PRODUCCIÓN DE RECURSOS (igual que PlayerController)
...
# 🔥 NUEVO: Cuando temina el modo batalla
func _on_battle_mode_ended() -> void:
	is_battle_mode = false
	print("🤖 [BotController] Modo batalla finalizado para %s" % player_name)
...
func add_worker() -> bool:
	...
	print("👷 [BotController] Worker añadido (Total: %d workers)" % workers)
	return true
...
func create_unit(unit_type: String) -> bool:
	# 🔥 CASO ESPECIAL: WORKER
	if unit_type == "train_worker" or unit_type == "train_slave":
		return add_worker()
